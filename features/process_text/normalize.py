from features.process_text.patterns import get_contraction_dict, get_special_characters_pattern,\
...
from features.process_text.correct_spelling import correct_word
...
def expand_emoticons(text):
    ...
    emoticons_pattern = compile('({})'.format(r'|'.join(get_emoticon_dict().keys())), flags=IGNORECASE | DOTALL)
...
def remove_stopwords_sentiment(text):
    ...
    while "very" in stop_set: stop_set.remove("very")
    while "no" in stop_set: stop_set.remove("no")
    while "not" in stop_set: stop_set.remove("not")
    while "nor" in stop_set: stop_set.remove("nor")
    while "same" in stop_set: stop_set.remove("same")
    while "more" in stop_set: stop_set.remove("more")
    while "most" in stop_set: stop_set.remove("most")
